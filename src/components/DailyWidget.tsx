...
export function DailyWidget() {
  ...
  // Clear the entry class once the animation has played.
  useEffect(() => {
    if (!entering) return;
    const id = setTimeout(() => setEntering(false), 520);
  ...
  return (
    <section className="card mt-3 overflow-hidden rounded-3xl">
      <button
        onClick={() => advance(1)}
        className="widget-press block w-full px-5 py-5 text-left"
      ...
      >
        ...
        <div key={index} className={entering ? 'widget-enter' : undefined}>
          ...
          <p className="mt-1.5 text-[13px] leading-relaxed text-[var(--ink-dim)]">
            {dhikr.meaning}
            {dhikr.note && <span className="ml-1.5 opacity-80">· {dhikr.note}</span>}
          </p>
        </div>
      </button>
    </section>
